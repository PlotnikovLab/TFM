function [lines,line_vars] = measure_line_force(bild_datei,stress,step,cutoff, line_vars)
...
function [u,b] = line_vec(L,step,cutoff,pos,vec)    %% L containd deltaX and deltaY for the line
    clear u b;    
    for i = 1:round(norm(L)/step)
            act_pos = (i-1)*step*Lnorm + [x(1),y(1)];   %WOW! that's cool. Benedikt calculates position in X and Y at the same time
            clear fk;
            fk = find(((pos(:,1) - act_pos(1)).^2 + (pos(:,2) - act_pos(2)).^2).^0.5 <= cutoff);    %it seems that area to average vectors is act_pos +/- cutoff, NOT 1/2 of cutoff
            if ~isempty(fk)  
