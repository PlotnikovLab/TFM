...
while 1
    if (strcmp(color_flag, 'multi_color'))
        ...
        quiver(strain.pos(idx(:),1),strain.pos(idx(:),2),strain.vec(idx(:),1),strain.vec(idx(:),2),mag,'r'); hold on;
        
        idx = find(strain.vec(:,1)>=0 & strain.vec(:,2)<0);
        quiver(strain.pos(idx(:),1),strain.pos(idx(:),2),strain.vec(idx(:),1),strain.vec(idx(:),2),mag,'g');
                    
        idx = find(strain.vec(:,1)<0 & strain.vec(:,2)>=0);
        quiver(strain.pos(idx(:),1),strain.pos(idx(:),2),strain.vec(idx(:),1),strain.vec(idx(:),2),mag,'m');
        
        idx = find(strain.vec(:,1)<0 & strain.vec(:,2)<0);
        quiver(strain.pos(idx(:),1),strain.pos(idx(:),2),strain.vec(idx(:),1),strain.vec(idx(:),2),mag,'b'); hold off;
    else
        quiver(strain.pos(:,1),strain.pos(:,2),strain.vec(:,1),strain.vec(:,2),mag,'r');
    end
