function  [startpos, pos, vec, precond_vec] = Xcorr_Track_Preestim_Beads(referenz_bild_datei, bild_datei, meshpos, meshvec, startpos, clust_shift, min_clust_size, max_clust_size)
        ...
        startpos = startpos(found,:);
        disp(['Could not track ', num2str(nnz(~found)),' beads.']);
end

function peak = fit_2Dgaussian_to_peak(bild,peak)
        peak = squeeze(peak);
        lbild = log(bild);
        dy = (lbild(peak(1)-1,peak(2)) - lbild(peak(1)+1,peak(2)))/2/(lbild(peak(1)-1,peak(2)) - 2*lbild(peak(1),peak(2)) + lbild(peak(1)+1,peak(2)));
        dx = (lbild(peak(1),peak(2)-1) - lbild(peak(1),peak(2)+1))/2/(lbild(peak(1),peak(2)-1) - 2*lbild(peak(1),peak(2)) + lbild(peak(1),peak(2)+1));
        peak = peak + [dy,dx];
end
