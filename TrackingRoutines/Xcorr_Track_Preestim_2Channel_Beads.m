...
function corr_mat = make_corr_mat(x_akt, y_akt,precond_vec,dref_b,db, shift_vec,clust_shift, clust_size)
            ...
            x_min=x_akt+precond_vec(2)+shift_vec(1);
            x_max=x_akt+precond_vec(2)+shift_vec(end)+clust_size-1;
            y_min=y_akt+precond_vec(1)+shift_vec(1);
            y_max=y_akt+precond_vec(1)+shift_vec(end)+clust_size-1;
            cmp_clust=db(x_min:x_max,y_min:y_max);
            output=normxcorr2(clust,cmp_clust);
            dimensions=size(clust);
            cmp_size=size(cmp_clust);
            corr_mat=output(dimensions(1):cmp_size(1),dimensions(2):cmp_size(2));
            %for s = shift_vec
            %    for z = shift_vec
