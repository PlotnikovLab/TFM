function  [pos, vec, enlarged, found, startpos] = TXcorr_Track_Preestim_2Channel_Beads(referenz_bild_datei1, bild_datei1, referenz_bild_datei2, bild_datei2, pre_pos, pre_vec, pos1, pos2, clust_shift, min_clust_size, max_clust_size, xshift_pos2, yshift_pos2)
        ...
        imagesc(b1);colormap gray, hold;
        %{
        scatter(pos2(:,1),pos2(:,2),'y.');
      
        p1 = pos2((pos2(:,1) < max_clust_size/2 + 10 + clust_shift) | (pos2(:,2) < max_clust_size/2 + 10 +clust_shift),:);
        p2 = pos2((pos2(:,1) > (size(dref_b2,2) - max_clust_size/2  - clust_shift-10)) | (pos2(:,2) > (size(dref_b2,1) - max_clust_size/2 -clust_shift-10)),:);
        scatter(p1(:,1),p1(:,2),'r.');
        scatter(p2(:,1),p2(:,2),'r.');
        %}
        ...
        pos1((pos1(:,1) < max_clust_size/2 + 10 + clust_shift) | (pos1(:,2) < max_clust_size/2 + 10 +clust_shift),:) = [];
        pos1((pos1(:,1) > (size(dref_b1,2) - max_clust_size/2  - clust_shift-10)) | (pos1(:,2) > (size(dref_b1,1) - max_clust_size/2 -clust_shift-10)),:) = [];

        pos2((pos2(:,1) < max_clust_size/2 + 10 + clust_shift) | (pos2(:,2) < max_clust_size/2 + 10 +clust_shift),:) = [];
        pos2((pos2(:,1) > (size(dref_b2,2) - max_clust_size/2  - clust_shift-10)) | (pos2(:,2) > (size(dref_b2,1) - max_clust_size/2 -clust_shift-10)),:) = [];
        ...
        for ip = 1:size(pos1,1)
            for clust_size = min_clust_size:2:max_clust_size
                ...
                rectangle('Position',[x_akt,y_akt,clust_size,clust_size]);
        ...
        disp(['Enlarged ',num2str(nnz(enlarged1)),' windows of ',num2str(size(pos1,1)),' total in channel 1.']);
        ...
        disp(['Enlarged ',num2str(nnz(enlarged2)),' windows of ',num2str(size(pos2,1)),' total in channel 2.']);
