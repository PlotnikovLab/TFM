function find_reference_and_register_images_bbs(ref_bild_dir, bild_dir, max_z_drift_per_frame, crop_image)
if nargin < 3
    max_z_drift_per_frame = 5;
end

result_folder = [ref_bild_dir,'_aligned_bbs'];
...
for t = 1:stacklen
    ...
    %----------------------------------------------------------------------
    rbild_clust_filtered = Gauss2D(rbild_clust,1,1);
    rbild_clust_background = Gauss2D(rbild_clust,10,1);
    rbild_clust = rbild_clust_filtered - rbild_clust_background;
    %----------------------------------------------------------------------
...
for f= 1:size(sorted_dir_names,1);
    ...
    %----------------------------------------------------------------------
    bild_clust_filtered = Gauss2D(bild_clust,1,1);
    bild_clust_background = Gauss2D(bild_clust,10,1);
    bild_clust = bild_clust_filtered - bild_clust_background;
